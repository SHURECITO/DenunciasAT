...
**Sesión 46 (2026-04-27):** migración Gemini AI Studio → Vertex AI: reemplazado `@google/generative-ai` por `@google/genai` (SDK unificado con Vertex AI mode + ADC); `libs/ai/src/gemini.service.ts` — clase `VertexModel` wrapper que preserva interfaz `model.generateContent()` existente, constructor usa `new GoogleGenAI({vertexai:true, project, location})`; `rag-service/rag.service.ts` — `ai.models.embedContent({model:'text-embedding-004', contents})` + `result.embeddings[0].values`, `ai.models.generateContent` para clasificación; eliminado `GEMINI_API_KEY` de todos los docker-compose y `warnMissingEnv`; añadido `GCP_REGION` a chatbot/document/rag en los tres composes; `deploy.yml` — nuevo job `grant-vertex-ai-role` (idempotente `roles/aiplatform.user`), detección de cambio de contraseña de BD con limpieza de volumen, `GEMINI_API_KEY` marcada como deprecada. Fix: `GCP_PROJECT_ID`+`GCP_REGION` añadidos a dashboard-api en ambos docker-compose (dev+prod); `chatbot-service/main.ts` — `NestFactory.create` envuelto en try/catch que loguea el mensaje real y llama `process.exit(1)` (causa raíz del crash "ExceptionHandler {}" era `new GoogleGenAI` lanzando síncronamente cuando `GCP_PROJECT_ID` está vacío). Bugs manuales: `createManual` en controller dispara fire-and-forget a document-service `/generar-desde-descripcion` (Bug 1); `resolveDestinatario` en document-builder añade fallback case-insensitive para encontrar titular aunque el nombre varíe en mayúsculas (Bug 2); formulario `/denuncias/nueva` añade campos barrio/comuna visibles + upload de evidencias JPG/PNG a GCS via `POST /denuncias/evidencias/upload` (Bug 3). CI/CD: job `verify` eliminado (firewall bloquea curl externo), `node-version: 22`, `docker/build-push-action@v6`, `submodules: false`+`lfs: false` en todos los checkouts.
