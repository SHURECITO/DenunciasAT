...
**Sesiones 35–36 (2026-04-20):** enriquecimiento semántico `dependencias.vector.db.json` (leyes por dependencia, stopwords); capa admisibilidad `InferenciasService.evaluarAdmisibilidad()` integrada en chatbot y document-service.
**Sesiones 37–39 (2026-04-21/22):** despliegue GCP: healthchecks, `docker-compose.prod.yml` Artifact Registry, CI/CD `deploy.yml`; migración MinIO → GCS (`GcsStorageService`, ADC, Signed URLs v4, `gs://` URLs internas).
**Sesiones 40–42 (2026-04-23):** auditoría QA: `imagenesEvidencia` en `api.ts`; frontend Dockerfile con ARG build-time; `/health` Next.js; hardening chatbot (try/catch 15s race Gemini); hardening Redis (retryStrategy, try/catch por operación); healthchecks en docker-compose.
**Sesiones 43–44 (2026-04-24):** CI/CD: Jest 30, secrets en build, v2 compose plugin, `COOKIE_SECURE`, `.env` regenerado en VM desde Secrets; pruning disco; `docker-compose.local-prod.yml`.
**Sesión 45 (2026-04-24):** sistema feedback: entidad `FeedbackDenuncia`, `POST/GET /feedback`, modal `ModalFeedback`, tab historial, tarjeta precisión IA en estadísticas.
**Sesión 46 (2026-04-27):** Vertex AI ADC: `@google/genai` SDK, `VertexModel` wrapper, embeddings 768D; GCP_REGION en todos los servicios; `createManual` fire-and-forget a document-service; `resolveDestinatario` case-insensitive; formulario nueva denuncia con barrio/comuna/evidencias GCS.
**Sesión 47 (2026-04-27):** fixes producción: `GCP_REGION` en local-prod dashboard-api; migration SQL `001_feedback_denuncias.sql` + paso en deploy.yml (exec psql); `createManual` retorna `documentoPendiente:true`; `errorDocumento` remueve guard `estado!==RECIBIDA` → usa `!incompleta`; roles Compute Engine SA en deploy.yml; MinIO → GCS en CLAUDE.md. `docker-compose.prod.yml` validado sin claves duplicadas (`docker compose config` exit 0); error GCP_PROJECT_ID en VM era del archivo desactualizado en `/opt/denunciasat/` (CI/CD no hace git pull de los compose). Deploy VM reforzado: `git reset --hard` antes de `.env`, verificación de credenciales Postgres antes de `down`, arranque de Postgres previo a migraciones con espera, y migración feedback habilita `pgcrypto`.
**Sesión 48 (2026-04-27):** diagnóstico del error de claves duplicadas en prod: `docker-compose.prod.yml` en `/opt/denunciasat/` estaba desactualizado (pre S46/47) porque el deploy no hace `git pull`; solución propuesta copiar compose actual o añadir paso de pull/scp; verificado `docker compose -f docker-compose.prod.yml config` exit 0 sin claves duplicadas.
**Sesión 49 (2026-04-28):** QA producción: fix bug "permanent-disable" en `rag-service` (`reindexarForzado` resetea `embeddingGeminiDisponible` para retry post-IAM-propagation); `GeminiService` null guard para `GCP_PROJECT_ID` vacío (no crash al arrancar sin Vertex AI); `docker-compose.local-prod.yml` monta credenciales ADC locales vía `GCP_ADC_FILE`; `test/verify-websockets.js` test WebSocket EIO4 con `ws`; fix `startsWith('40/eventos,')` en test (servidor envía JSON tras ack namespace); GCP_ADC_FILE apunta a `%APPDATA%/gcloud/application_default_credentials.json` en .env local; V1–V7 todos pasan.
**Sesión 52 (2026-04-29):** fix CI/CD: `deploy.yml` agrega `sudo chmod 644 / sudo chown $USER` sobre `.env` al inicio del bloque SSH, antes de `cat > .env`, para corregir "Permission denied" cuando el archivo queda con permisos root de deploys anteriores.
**Sesión 51 (2026-04-29):** fix webhook Evolution API: `validarFirmaWebhook` ya no rechaza cuando no hay cabecera `x-webhook-signature` (Evolution v2.2.3 no la envía por defecto); solo rechaza si el secret está configurado Y la firma llega Y no coincide. Removida validación de timestamp (Evolution no lo envía). Tests 12/12.
**Sesión 50 (2026-04-29):** corrección 3 bugs: (1) document-service: logs de progreso en `generarDocumento` (pasos Gemini/build/GCS); dashboard-api `generarDocumento` endpoint ya no swallow errores de conexión a document-service. (2) WebSocket QR: `socket.ts` deriva URL del `window.location.hostname` con puerto 8741 cuando `NEXT_PUBLIC_WS_URL` apunta a localhost en browser de producción; `test/verify-qr-whatsapp.js` nuevo script QR/estado. (3) editor de dependencias: `getDependencias()` expone `nombreTitular`; `ModalEditarDenuncia` muestra `nombreTitular` (nombre del titular) en vez de `cargoTitular` como subtítulo.
**Sesión 53 (2026-04-29):** 4 fixes producción: (1) `GeminiService.procesarMensajeChatbot` — log detallado de errores Vertex AI (code, stack) para diagnóstico de fallos en WhatsApp. (2) `descargarDocumento` controller — logs cuando `documentoUrl` es null o GCS falla al generar signed URL. (3) `deploy.yml` — verificación post-deploy (≥8 servicios up o falla el CI/CD) + auto-seed idempotente del admin (SEED_ENABLED=true → seed → false, restart dashboard-api). (4) Tests nuevos: `libs/ai/src/gemini.service.spec.ts` (fallback logging) y `apps/dashboard-api/src/denuncias/denuncias.controller.spec.ts` (descarga documento, 28/28 pasan).
**Sesión 54 (2026-04-29):** fix crítico deploy: `deploy.yml` — (1) `.env` permission denied: `DEPLOY_USER=$(whoami)`, `sudo chown || true` + `sudo chmod || true` antes de escribir, `sudo tee .env` en lugar de `cat >` para que funcione aunque el archivo sea root, `sudo chown ${DEPLOY_USER}` inmediatamente después de ENVEOF. (2) `restart: unless-stopped` ya presente en todos los servicios vía `x-service-defaults` — sin cambios requeridos en docker-compose.prod.yml.
**Sesión 55 (2026-04-29):** verificación NEXT_PUBLIC_API_URL — deploy.yml pasa `NEXT_PUBLIC_API_URL=${{ secrets.NEXT_PUBLIC_API_URL }}` como build-arg al frontend (línea 177); Dockerfile declara `ARG` + `ENV` correctamente. Sin cambios necesarios; rebuild disparado por este commit.
**Sesión 56 (2026-05-04):** 7 fixes: (1) historial mensajes secuencial (Promise.allSettled → async loop) para orden determinista en PostgreSQL. (2) MSG_BIENVENIDA institucional sin emoji. (3) @lid: resolución siempre, sin guard `>13 dígitos`. (4) SYSTEM_PROMPT_CHATBOT sección EVIDENCIA: pregunta activa por fotos después de descripción/ubicación. (5) `sameSite: 'lax'` en cookie login (fix feedback modal). (6) modelLegal `maxOutputTokens: 8000`; generarAsunto inline `maxOutputTokens: 4096`. (7) deploy.yml: `up -d --remove-orphans` al finalizar + verificación final 9/10 servicios.
